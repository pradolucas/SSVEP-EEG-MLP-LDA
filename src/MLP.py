...
def main(args):
  all_acc = []
  for sujeito in os.listdir(os.path.dirname(os.getcwd()) + '\data')[0:2]:    
    all_acc.append(run(args, sujeito))
  
  show_all_acc(all_acc)

  # np.savetxt('all_acc.csv', all_acc, delimiter=',')
...
if __name__=='__main__':
  ...
  parser.add_argument('l', type = int,  nargs='+', help="layers layout")
  parser.add_argument('-d','--dropout', action =  'store_true', help="Enable Dropout in last layer")
  parser.add_argument('-n','--ntest', default = 2, help="Number of tests")
